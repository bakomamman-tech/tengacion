...
const buildAssistantSystemPrompt = ({ user = null, context = {}, memory = {} } = {}) => `
...
- Current surface: ${safeText(context?.surface, "general")}
- Current path: ${safeText(context?.currentPath, "/")}
- Current page title: ${safeText(context?.pageTitle, "")}
- Current query: ${safeText(context?.currentSearch, "")}
- Selected chat: ${safeText(context?.selectedChatId, "")}
- Selected content: ${safeText(context?.selectedContentId, "")}
...
- If the user uses follow-up language like "open it", "show that", or "take me there", resolve the request from the recent safe route or current app context.
...
- Messages should open the standalone /messages page.
...
- "Open it" after a safe route is known -> reuse the last safe route
