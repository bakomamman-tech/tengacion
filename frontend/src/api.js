...
// ======================================================
// KADAHIVE INSTITUTION PORTAL
// ======================================================

export const loadKadahivePublic = () =>
  request(`${API_BASE}/kadahive/public`, {
    suppressAuthFailure: true,
  });

export const joinKadahive = () =>
  request(`${API_BASE}/kadahive/membership/join`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });

export const loadKadahiveMember = () =>
  request(`${API_BASE}/kadahive/me`);

export const createKadahiveBooking = (payload = {}) =>
  request(`${API_BASE}/kadahive/bookings`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const registerForKadahiveEvent = (eventId) =>
  request(`${API_BASE}/kadahive/events/${encodeURIComponent(eventId || "")}/register`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}),
  });

export const loadKadahiveAdmin = () =>
  request(`${API_BASE}/kadahive/admin/overview`);

export const loadKadahiveMembers = (search = "") =>
  request(
    `${API_BASE}/kadahive/admin/members${
      search ? `?search=${encodeURIComponent(search)}` : ""
    }`
  );

export const updateKadahiveMember = (userId, payload = {}) =>
  request(`${API_BASE}/kadahive/admin/members/${encodeURIComponent(userId || "")}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const createKadahiveEvent = (payload = {}) =>
  request(`${API_BASE}/kadahive/admin/events`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const updateKadahiveEvent = (eventId, payload = {}) =>
  request(`${API_BASE}/kadahive/admin/events/${encodeURIComponent(eventId || "")}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const deleteKadahiveEvent = (eventId) =>
  request(`${API_BASE}/kadahive/admin/events/${encodeURIComponent(eventId || "")}`, {
    method: "DELETE",
  });

export const createKadahiveResource = (payload = {}) =>
  request(`${API_BASE}/kadahive/admin/resources`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const updateKadahiveResource = (resourceId, payload = {}) =>
  request(`${API_BASE}/kadahive/admin/resources/${encodeURIComponent(resourceId || "")}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });

export const deleteKadahiveResource = (resourceId) =>
  request(`${API_BASE}/kadahive/admin/resources/${encodeURIComponent(resourceId || "")}`, {
    method: "DELETE",
  });

export const updateKadahiveBooking = (bookingId, payload = {}) =>
  request(`${API_BASE}/kadahive/admin/bookings/${encodeURIComponent(bookingId || "")}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export const updateCreatorAutomationRunControl = (runId, payload = {}) =>
  request(`${API_BASE}/creator/automation/runs/${encodeURIComponent(runId || "")}/control`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      ...getAuthHeaders(),
...
export const updateCreatorWorkflowRunControl = (runId, payload = {}) =>
  request(`${API_BASE}/creator/orchestration/workflows/${encodeURIComponent(runId || "")}/control`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      ...getAuthHeaders(),
...
export const adminGetAnalyticsCommerceOps = (params = {}) => {
  const query = new URLSearchParams();
  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {query.set(key, String(value));}
  });
  return request(`${API_BASE}/admin/analytics/commerce-ops?${query.toString()}`, {
    headers: getAuthHeaders(),
    timeoutMs: 45000,
  });
};
...
export const adminGetAutomationOrchestrationOperatingSystem = (params = {}) => {
  const query = new URLSearchParams();
  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {query.set(key, String(value));}
  });
  return request(`${API_BASE}/admin/analytics/automation-orchestration-operating-system?${query.toString()}`, {
    headers: getAuthHeaders(),
    timeoutMs: 45000,
  });
};
...
export const adminGetAnalyticsReliabilityHealth = (params = {}) => {
  ...
  return request(`${API_BASE}/admin/analytics/reliability-health?${query.toString()}`, {
    headers: getAuthHeaders(),
    timeoutMs: 45000,
  });
};
...
export const adminGetRevenueLedger = (params = {}) => {
  const query = new URLSearchParams();
  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {query.set(key, String(value));}
  });
  return request(`${API_BASE}/admin/finance/revenue-ledger?${query.toString()}`, {
    headers: getAuthHeaders(),
    timeoutMs: 45000,
  });
};
