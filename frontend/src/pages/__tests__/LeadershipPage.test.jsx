...
describe("LeadershipPage", () => {
  it("presents only verified public team members", () => {
    ...
    expect(
      screen.getByRole("navigation", { name: "Public Tengacion navigation" })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("heading", { name: "Verified team members" })
    ).toBeInTheDocument();
    expect(screen.getAllByText("Stephen Daniel Kurah")[0]).toBeInTheDocument();
    expect(
      screen.getByText("Founder, Chairman and Chief Executive Officer")
    ).toBeInTheDocument();
    const founderPortraits = screen.getAllByAltText(
      /Stephen Daniel Kurah, Founder/i
    );
    ...
    expect(
      screen.getByRole("heading", { name: "Diana Comfort Danjuma" })
    ).toBeInTheDocument();
    expect(
      screen.getByAltText(
        "Diana Comfort Danjuma, Social Media Lead at Tengacion"
      )
    ...
    expect(
      screen.getByRole("heading", { name: "Vincent Bilat Danjuma" })
    ).toBeInTheDocument();
    expect(
      screen.getByAltText(
    ...
    expect(
      screen.getByAltText(
        "Christopher Ebere Chibuzor, Abuja Creators Support Lead at Tengacion"
      )
    ).toHaveAttribute(
      "src",
    ...
    expect(
      screen.getByRole("heading", { name: "Tengacion Intern" })
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Illustrative placeholder")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Appointment to be announced")
    ).not.toBeInTheDocument();
  });
});
