...
describe("Posts feed", () => {
  ...
  test("POST /api/posts uploads up to eight mixed photos and videos", async () => {
    let uploadRequest = request(app)
      .post("/api/posts")
      .set("Authorization", `Bearer ${authToken}`)
      .field("text", "Mixed event highlights");

    const attachments = [
      { filename: "clip-1.mp4", contentType: "video/mp4", body: "video-1", type: "video" },
      { filename: "photo-1.png", contentType: "image/png", body: "image-1", type: "image" },
      { filename: "photo-2.jpg", contentType: "image/jpeg", body: "image-2", type: "image" },
      { filename: "clip-2.webm", contentType: "video/webm", body: "video-2", type: "video" },
      { filename: "photo-3.webp", contentType: "image/webp", body: "image-3", type: "image" },
      { filename: "photo-4.png", contentType: "image/png", body: "image-4", type: "image" },
      { filename: "clip-3.mov", contentType: "video/quicktime", body: "video-3", type: "video" },
      { filename: "photo-5.png", contentType: "image/png", body: "image-5", type: "image" },
    ];

    attachments.forEach((attachment) => {
      uploadRequest = uploadRequest.attach("file", Buffer.from(attachment.body), {
    ...
    expect(response.body).toMatchObject({
      text: "Mixed event highlights",
      type: "image",
    });
    expect(response.body.media).toHaveLength(8);
    expect(response.body.media.map((entry) => entry.type)).toEqual(
      attachments.map((attachment) => attachment.type)
    );

    const stored = await Post.findById(response.body._id).lean();
    expect(stored).toBeTruthy();
    expect(stored.media).toHaveLength(8);
    expect(stored.media.map((entry) => entry.type)).toEqual(
      attachments.map((attachment) => attachment.type)
    );
    expect(cloudinary.uploader.upload_stream).toHaveBeenCalledTimes(8);
    expect(cloudinary.uploader.upload_stream).toHaveBeenCalledWith(
      expect.objectContaining({
        folder: "tengacion/posts/images",
        resource_type: "image",
  ...
  test("POST /api/posts rejects more than eight uploaded photos and videos", async () => {
    let uploadRequest = request(app)
      .post("/api/posts")
      .set("Authorization", `Bearer ${authToken}`)
      .field("text", "Too many attachments");

    for (let index = 0; index < 9; index += 1) {
      uploadRequest = uploadRequest.attach("file", Buffer.from(`image-${index + 1}`), {
    ...
    expect(response.body.message).toBe("Too many files were uploaded for this field.");
  });
