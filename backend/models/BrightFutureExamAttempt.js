...
const AttemptQuestionSchema = new mongoose.Schema(
  {
    ...
    order: { type: Number, required: true, min: 1, max: 50 },
...
const BrightFutureExamAttemptSchema = new mongoose.Schema(
  {
    ...
    questions: {
      ...
      required: true,
      validate: {
        validator: (value) => Array.isArray(value) && [40, 50].includes(value.length),
        message: "A Bright Future attempt must contain 40 legacy questions or 50 current questions.",
      },
    },
    currentQuestionIndex: { type: Number, default: 0, min: 0, max: 50 },
