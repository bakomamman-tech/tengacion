      ...
      <div className="creator-profile-summary-head">
        <div className="creator-avatar-mark">
          {avatarSrc ? (
            <img
              src={avatarSrc}
              alt={creatorProfile?.displayName || creatorProfile?.fullName || "Creator"}
            />
          ) : (
            (creatorProfile?.displayName || creatorProfile?.fullName || "C")
              .slice(0, 1)
              .toUpperCase()
          )}
        </div>
        ...
        <div>
          ...
          <p>
            {creatorLanes.length
              ? creatorLanes.map((entry) => formatCreatorLaneLabel(entry)).join(" | ")
              : "Creator workspace"}
          </p>
        </div>
      </div>
