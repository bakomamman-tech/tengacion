...
export default function GsiResearchDashboard() {
  ...
  const [payload, setPayload] = useState({ results: [], pagination: { total: 0, pages: 1 } });
  ...
  return (
    <div className="gsi-app-shell gsi-dashboard-page">
      ...
      <main className="gsi-dashboard-main">
        <section className="gsi-dashboard-hero"><div><div className="gsi-eyebrow"><GsiIcon name="globe" size={15} /> Public research dashboard</div><h1>Research you can find, understand, and trace.</h1><p>Browse paper-level submissions and journal records with visible evidence, transparent scores, and no prestige-based ranking.</p></div><div className="gsi-dashboard-total"><strong>{formatNumber(payload.pagination.total)}</strong><span>public records</span></div></section>
        <form className="gsi-dashboard-filters" onSubmit={search}>
          <label className="gsi-dashboard-search"><GsiIcon name="search" size={19} /><input value={draft.q} onChange={update("q")} placeholder="Search title, author, abstract, or field" aria-label="Search research" /></label>
          <select value={draft.type} onChange={update("type")} aria-label="Record type"><option value="">All record types</option><option value="paper">Papers</option><option value="journal">Journals</option></select>
        ...
        {!loading && !error && payload.results.length ? <section className="gsi-research-grid" aria-label="Research records">{payload.results.map((record) => <article className="gsi-research-card" key={record.archiveId}><div className="gsi-research-card-top"><span className={`gsi-record-kind is-${record.recordKind}`}><GsiIcon name={record.recordKind === "paper" ? "file" : "book"} size={14} /> {record.recordKind}</span><div className="gsi-card-score"><strong>{record.gsiScore}</strong><small>/100</small></div></div><h2><Link to={record.publicRecordPath}>{record.title}</Link></h2><p className="gsi-research-byline">{record.subtitle || (record.recordKind === "journal" ? "Publisher not listed" : "Authors not listed")}</p>{record.abstract ? <p className="gsi-research-abstract">{record.abstract}</p> : null}<dl><div><dt>Field</dt><dd>{record.field || "Multidisciplinary"}</dd></div><div><dt>Country</dt><dd>{formatCountry(record.countryCode)}</dd></div>{record.publicationYear ? <div><dt>Year</dt><dd>{record.publicationYear}</dd></div> : null}</dl><div className="gsi-research-card-footer"><span><GsiIcon name="shield" size={14} /> {record.impactEvidenceStatus === "self-reported" ? "Impact evidence disclosed" : "No impact claim"}</span><Link to={record.publicRecordPath}>View record <GsiIcon name="arrow" size={14} /></Link></div></article>)}</section> : null}
