        ...
        {items.slice(0, 6).map((video) => (
          <div key={video.id} className={styles.itemCard}>
            ...
            <div style={{ display: "flex", gap: "0.35rem", marginTop: "0.45rem" }}>
              <button type="button" className={cx(buttonStyles({ variant: "secondary", size: "sm" }), styles.playBtn)} onClick={() => onPlay(video, items)}>Play</button>
              {!video.isFree ? (
                <button type="button" className={cx(buttonStyles({ variant: "primary", size: "sm" }), styles.buyBtn)} onClick={() => onCheckout("video", video.id)}>Unlock</button>
              ) : null}
              <button type="button" className={cx(buttonStyles({ variant: "icon", size: "sm", iconOnly: true }), styles.menuBtn)} onClick={() => onMenu(video)} aria-label={`More options for ${video.title}`}>...</button>
            </div>
          </div>
        ))}
      </div>
