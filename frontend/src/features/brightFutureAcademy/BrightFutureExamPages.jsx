...
export function BrightFutureExamInstructionsPage() {
  ...
  return (
    <BrightFutureLayout portal activeKey="exam">
      ...
      <div className="bfa-exam-summary"><article><span>▥</span><strong>40</strong><small>Total questions</small></article><article><span>◇</span><strong>4</strong><small>Academic subjects</small></article><article><span>◷</span><strong>{competition?.questionTimerSeconds || 50}s</strong><small>Per question</small></article><article><span>1×</span><strong>One</strong><small>Official attempt</small></article></div>
      <section className="bfa-instruction-panel">
        ...
        <div className="bfa-rule-grid">
          {[
            ["01", "Four balanced subjects", "Mathematics, English Language, Basic Science & Technology, and Social Studies each contain 10 questions."],
...
export function BrightFutureResultPage() {
  ...
  const subjectScores = useMemo(() => result ? SUBJECTS.map((subject) => ({ ...subject, score: result.subjectScores[subject.key] || 0 })) : [], [result]);
  ...
  return (
    <BrightFutureLayout portal activeKey="result">
      <section className="bfa-result-hero"><div className="bfa-result-confetti" aria-hidden="true">✦ ● ◆ ✦ ●</div><span className="bfa-result-medal">★</span><p className="bfa-eyebrow">Examination complete</p><h1>Congratulations, {result.candidate.firstName}!</h1><p>Your official answers have been scored securely. This is your current verified competition result.</p><div className="bfa-result-score"><strong>{result.totalScore}</strong><span>/40<small>Total score</small></span><i>{result.percentage}%</i></div><div className="bfa-result-rank"><small>Current rank</small><strong>#{result.rank || "—"}</strong></div></section>
