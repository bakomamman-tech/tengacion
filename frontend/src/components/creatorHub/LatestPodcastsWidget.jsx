...
export default function LatestPodcastsWidget({ podcasts = [], onPlay, onViewAll, onCheckout, currencyMode = "NG" }) {
  const formatPrice = (item) => {
    ...
    if (amount <= 0 || item.isFree) return "Free";
      ...
      {podcasts.length ? (
        podcasts.slice(0, 8).map((podcast) => (
          <div key={podcast.id} className={styles.trackRow}>
            ...
            <button type="button" className={styles.playBtn} onClick={() => onPlay(podcast, podcasts)}>Play</button>
            <span className={styles.mutedText}>{formatPrice(podcast)}</span>
            {!podcast.isFree ? (
              <button type="button" className={styles.buyBtn} onClick={() => onCheckout("podcast", podcast.id)}>Buy</button>
            ) : null}
          </div>
        ))
      ) : (
