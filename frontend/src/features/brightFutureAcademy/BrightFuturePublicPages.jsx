...
export function BrightFutureLeaderboardPage() {
  ...
  return (
    <BrightFutureLayout portal={Boolean(candidate)} activeKey="leaderboard">
      ...
      <header className="bfa-public-page-heading"><p className="bfa-eyebrow">Live national standings</p><h1>Competition leaderboard</h1><p>Rankings come only from completed database attempts: highest percentage, then STEM, English and shortest legitimate completion time.</p></header>
      {data.leader ? <section className="bfa-champion-card"><div className="bfa-champion-card__stars" aria-hidden="true">✦ ◆ ✦</div><span>♛</span><div><small>{data.competition?.competitionStatus === "results_published" ? "Bright Future Academy CBT Champion" : "Current Overall Leader"}</small><h2>{data.leader.displayName}</h2><p>{data.leader.classLevel} · {data.leader.schoolName} · {data.leader.state}</p></div><strong>{data.leader.score}<small>/{data.leader.maximumScore || 50}</small><i>{data.leader.percentage}%</i></strong></section> : null}
      <section className="bfa-leaderboard-panel">
        ...
        {loading ? <div className="bfa-table-loader"><span /><p>Loading verified standings…</p></div> : (
          <div className="bfa-leaderboard-table" role="table" aria-label="Competition leaderboard">
            ...
            {data.entries.map((entry) => <div className={`bfa-leaderboard-row ${entry.rank <= 3 ? `is-top is-place-${entry.rank}` : ""}`} role="row" key={`${entry.rank}-${entry.candidateId}`}><span><i>{entry.rank <= 3 ? ["🥇", "🥈", "🥉"][entry.rank - 1] : `#${entry.rank}`}</i></span><span><strong>{entry.displayName}</strong><small>{entry.candidateId}</small></span><span>{entry.classLevel}</span><span><strong>{entry.schoolName}</strong><small>{entry.state}</small></span><span><strong>{entry.score}/{entry.maximumScore || 50}</strong><small>{entry.percentage}%</small></span><span>{formatDuration(entry.completionTime)}</span></div>)}
