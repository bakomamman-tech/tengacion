...
const MODERATION_PROVIDER = String(
  process.env.MODERATION_PROVIDER
    || (process.env.NODE_ENV === "test" ? "internal_heuristics" : "openai")
).trim().toLowerCase();
const MODERATION_ENABLED = String(process.env.MODERATION_ENABLED || "true").toLowerCase() !== "false";
const MODERATION_MODEL = String(
  process.env.MODERATION_MODEL || "omni-moderation-latest"
).trim();
const MODERATION_VISION_MODEL = String(
  process.env.MODERATION_VISION_MODEL
    || process.env.OPENAI_MODEL_FAST
    || process.env.OPENAI_MODEL
    || "gpt-5.4-mini"
).trim();
const MODERATION_CONTEXTUAL_VISION_ENABLED =
  String(process.env.MODERATION_CONTEXTUAL_VISION_ENABLED || "true").toLowerCase() !== "false";
const MODERATION_REQUEST_TIMEOUT_MS = Math.max(
  1000,
  Number(process.env.MODERATION_REQUEST_TIMEOUT_MS || 15000)
);
...
const REVIEW_THRESHOLD = readThreshold("MODERATION_REVIEW_THRESHOLD", 0.8);
...
const MODERATION_REPEAT_VIOLATOR_STRIKE_THRESHOLD = Math.max(
  1,
  Number(process.env.MODERATION_REPEAT_VIOLATOR_STRIKE_THRESHOLD || 5)
);
