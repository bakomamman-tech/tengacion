...
import {
  ...
  adminResetBrightFuturePassword,
...
const subjectName = (key = "") => ({ nigerian_entertainment: "Nigerian Music & Movies", nigerianEntertainment: "Nigerian Music & Movies", football: "Football", technology: "Tech, Tengacion & Silicon Valley", general_english: "General English", generalEnglish: "General English", stem: "Mathematics & Science/STEM", mathematics: "Mathematics", english: "English", basic_science_technology: "Basic Science & Technology", basicScienceTechnology: "Basic Science & Technology", social_studies: "Social Studies", socialStudies: "Social Studies" }[key] || key);
const scoreBreakdown = (scores = {}) => Object.entries(scores).filter(([, value]) => Number(value) > 0);
...
function OverviewTab({ overview }) {
  const maximumScore = overview.maximumScore || 50;
  const metrics = [
    ["Total registrations", overview.totalRegistrations], ["Completed exams", overview.totalCompleted],
    ["Currently in progress", overview.inProgress], ["Average score", `${overview.averageScore || 0}/${maximumScore}`],
    ["Highest score", `${overview.highestScore || 0}/${maximumScore}`], ["Lowest score", `${overview.lowestScore || 0}/${maximumScore}`],
...
function StudentsTab({ payload, loading, filters, setFilters, applyFilters, onStatus, onReset, onRecover, saving }) {
  return <section className="adminx-panel"><div className="adminx-panel-head"><div><h2 className="adminx-panel-title">Registered students & access recovery</h2><span className="adminx-section-meta">{number(payload.total)} permanent MongoDB records · search to recover a BFA reference or reset a password</span></div></div><form className="bfa-admin-filters" onSubmit={applyFilters}><input className="adminx-input" value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Search name, BFA reference, phone, school or state" /><select className="adminx-select" value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value }))}><option value="">All statuses</option><option value="active">Active</option><option value="disabled">Disabled</option><option value="withdrawn">Withdrawn</option></select><select className="adminx-select" value={filters.completed} onChange={(event) => setFilters((current) => ({ ...current, completed: event.target.value }))}><option value="">Any exam state</option><option value="true">Completed</option><option value="false">Not completed</option></select><button className="adminx-btn adminx-btn--primary">Apply</button></form>{loading ? <div className="adminx-loading">Loading Bright Future students…</div> : <div className="adminx-table-wrap adminx-table-wrap--flush"><table className="adminx-table bfa-admin-table"><thead><tr><th>Candidate</th><th>Class / School</th><th>Contact (private)</th><th>Exam</th><th>Score / Rank</th><th>Integrity</th><th>Actions</th></tr></thead><tbody>{(payload.students || []).map((student) => <tr key={student.id}><td><strong>{student.fullName}</strong><small>{student.candidateId}</small><span className={`adminx-badge is-${student.status}`}>{student.status}</span></td><td><strong>{student.classLevel}</strong><small>{student.schoolName}</small><small>{student.state} · {student.lga}</small></td><td><small>Guardian: {student.guardianPhone || "—"}</small><small>Student: {student.studentPhone || "—"}</small><small>Access: {student.credentialMode === "password" ? "Password" : "Legacy guardian phone"}</small></td><td><strong>{student.competitionStatus.replaceAll("_", " ")}</strong><small>Attempt {student.attemptNumber || 0}</small><small>{date(student.examCompletedAt || student.examStartedAt)}</small></td><td><strong>{student.examCompleted ? `${student.totalScore}/${student.maximumScore || 50} · ${student.percentage}%` : "—"}</strong><small>Rank {student.ranking ? `#${student.ranking}` : "—"}</small></td><td><strong>{student.violationCount || 0} warning(s)</strong><small>{student.submissionReason || "No submission"}</small></td><td><div className="bfa-admin-actions"><button type="button" disabled={saving} onClick={() => onRecover(student)}>Recover access</button><button type="button" disabled={saving} onClick={() => onStatus(student)}>{student.status === "active" ? "Disable" : "Restore"}</button><button type="button" disabled={saving} onClick={() => onReset(student)}>Reset / retake</button></div></td></tr>)}</tbody></table>{!payload.students?.length ? <div className="adminx-empty">No students match these filters.</div> : null}</div>}</section>;
}

function ResultsTab({ payload, loading }) {
  return <section className="adminx-panel"><div className="adminx-panel-head"><div><h2 className="adminx-panel-title">Verified examination results</h2><span className="adminx-section-meta">Student → attempt → category scores → rank → integrity status</span></div></div>{loading ? <div className="adminx-loading">Loading results…</div> : <div className="adminx-table-wrap adminx-table-wrap--flush"><table className="adminx-table bfa-admin-results"><thead><tr><th>Rank</th><th>Candidate</th><th>Class</th><th>School</th><th>Category scores</th><th>Total</th><th>%</th><th>Time</th><th>Status</th></tr></thead><tbody>{(payload.results || []).map((result) => <tr key={result.id}><td><strong>#{result.ranking || "—"}</strong></td><td><strong>{result.fullName}</strong><small>{result.candidateId}</small></td><td>{result.classLevel}</td><td>{result.schoolName}</td><td>{scoreBreakdown(result.subjectScores).map(([key, value]) => <small key={key}>{subjectName(key)}: {value}/10</small>)}</td><td><strong>{result.totalScore}/{result.maximumScore || 50}</strong></td><td>{result.percentage}%</td><td>{time(result.totalTimeUsed)}</td><td><span className={`adminx-badge ${result.violationCount ? "is-warning" : "is-success"}`}>{result.submissionReason || "completed"}</span><small>{result.violationCount || 0} violation(s)</small></td></tr>)}</tbody></table>{!payload.results?.length ? <div className="adminx-empty">No completed examination results yet.</div> : null}</div>}</section>;
}

function LeaderboardTab({ leaderboard }) {
  return <section className="adminx-panel"><div className="adminx-panel-head"><div><h2 className="adminx-panel-title">Live server ranking</h2><span className="adminx-section-meta">Percentage → STEM → English → shortest legitimate time</span></div>{leaderboard.hidden ? <span className="adminx-badge is-warning">Public board hidden</span> : null}</div><div className="bfa-admin-leaderboard">{(leaderboard.entries || []).map((entry) => <article key={`${entry.rank}-${entry.candidateId}`}><span>{entry.rank <= 3 ? ["🥇", "🥈", "🥉"][entry.rank - 1] : `#${entry.rank}`}</span><div><strong>{entry.displayName}</strong><small>{entry.classLevel} · {entry.schoolName} · {entry.state}</small></div><b>{entry.score}/{entry.maximumScore || 50} <small>{entry.percentage}% · {time(entry.completionTime)}</small></b></article>)}{!leaderboard.entries?.length ? <div className="adminx-empty">No live ranking records yet.</div> : null}</div></section>;
}

function RecoveryDialog({ recovery, onClose }) {
  if (!recovery) {return null;}
  const copy = (value) => navigator.clipboard?.writeText(value).catch(() => null);
  return <div className="bfa-admin-modal-backdrop"><section className="bfa-admin-recovery-modal" role="dialog" aria-modal="true" aria-labelledby="bfa-recovery-title"><p className="adminx-section-meta">Administrator-assisted access recovery</p><h2 id="bfa-recovery-title">New student credentials</h2><p>Share these details privately with <strong>{recovery.fullName}</strong>. The password is shown only in this dialog.</p><dl><div><dt>Candidate reference</dt><dd><code>{recovery.candidateId}</code><button type="button" onClick={() => copy(recovery.candidateId)}>Copy</button></dd></div><div><dt>New password</dt><dd><code>{recovery.temporaryPassword}</code><button type="button" onClick={() => copy(recovery.temporaryPassword)}>Copy</button></dd></div></dl><div className="bfa-admin-recovery-warning">Never send credentials in a public post or group chat.</div><button type="button" className="adminx-btn adminx-btn--primary" onClick={onClose}>I have saved the credentials</button></section></div>;
}
...
export default function AdminBrightFutureAcademy({ user }) {
  ...
  const recoverAccess = async (student) => {
    if (!window.confirm(`Issue a new password for ${student.fullName}? Their previous password will stop working.`)) {return;}
    setSaving(true); setError(""); setNotice(""); setRecovery(null);
    try {
      const data = await adminResetBrightFuturePassword(student.id, { reason: "Administrator-assisted access recovery" });
      setRecovery({ ...data.credentials, fullName: student.fullName });
      setNotice("Student password reset. Save the new credentials from the private dialog.");
      await load();
    } catch (requestError) {
      setError(requestError.message || "Access recovery failed. A fresh step-up verification may be required.");
  ...
  return <AdminShell title="Bright Future Academy" subtitle="Smart School Portal and National CBT Challenge operations." user={user} actions={<button type="button" className="adminx-btn" onClick={load}>Refresh</button>}><AdminNotice error={error} notice={notice} /><nav className="bfa-admin-tabs" aria-label="Bright Future Academy admin sections">{TABS.map(([key, label]) => <button type="button" key={key} className={tab === key ? "is-active" : ""} onClick={() => setTab(key)}>{label}</button>)}</nav>{tab === "overview" ? <OverviewTab overview={overview} /> : null}{tab === "students" ? <StudentsTab payload={students} loading={loading} filters={filters} setFilters={setFilters} applyFilters={(event) => { event.preventDefault(); setAppliedFilters(filters); }} onStatus={updateStatus} onReset={resetAttempt} onRecover={recoverAccess} saving={saving} /> : null}{tab === "results" ? <ResultsTab payload={results} loading={loading} /> : null}{tab === "leaderboard" ? <LeaderboardTab leaderboard={leaderboard} /> : null}{tab === "questions" ? <QuestionsTab questions={questions} onSave={saveQuestion} saving={saving} /> : null}{tab === "controls" ? <ControlsTab controls={controls} setControls={setControls} onSave={saveControls} saving={saving} /> : null}<RecoveryDialog recovery={recovery} onClose={() => setRecovery(null)} /></AdminShell>;
}
